...
const emptyMachine = { machineCode: "", machineName: "", machineType: "Cutter", section: "Cutting", capacityPerHour: 0, maintenanceStatus: "Good" };
...
export default function ProductionSetupPage({ notify, mode = "both" }) {
  ...
  return <>
    ...
    <div className="production-two-column">
      {mode !== "employee" && <Card title="Add Cutter / Spreader Machine"><form className="compact-form" onSubmit={submitMachine}>{Object.keys(emptyMachine).map((key) => <label key={key}><span>{key}</span>{key === "machineType" ? <select value={machine[key]} onChange={(event) => setMachine({ ...machine, [key]: event.target.value })}><option>Cutter</option><option>Spreader</option></select> : <input required={["machineCode", "machineName"].includes(key)} type={key === "capacityPerHour" ? "number" : "text"} value={machine[key]} onChange={(event) => setMachine({ ...machine, [key]: event.target.value })} />}</label>)}<button className="primary">Save Machine & Create QR</button></form></Card>}
    ...
    {mode !== "employee" && <Card title="Created Machine QR Codes"><DataTable rows={machines.filter((row) => ["Cutter", "Spreader"].includes(row.machineType))} columns={[{key:"machineQr",label:"Machine QR",render:(row)=><QRGenerator size={72} value={`${window.location.origin}/production?machineCode=${encodeURIComponent(row.machineCode)}`}/>},{key:"machineCode",label:"Code"},{key:"machineName",label:"Machine"},{key:"machineType",label:"Type"},{key:"status",label:"Status"},{key:"actions",label:"QR Download",render:(row)=><div className="table-actions"><button type="button" onClick={()=>printMasterQrPdf("machine",row)}>Print</button><button type="button" className="primary" onClick={()=>downloadMasterQrPdf("machine",row)}>Download</button></div>}]} /></Card>}
