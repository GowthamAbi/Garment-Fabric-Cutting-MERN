...
export function allocatePiecesByStock(totalPcs, perPieceKg, stockByColour) {
  ...
  if (!Number.isInteger(pcs) || pcs <= 0 || weight <= 0 || !stockByColour.length)
    throw new TypeError("Valid PCS, piece weight and selected colours are required");

  const target = distributePieces(pcs, stockByColour.length);
  const allocations = stockByColour.map((stock, index) => {
    const capacity = Math.floor((Number(stock.availableWeightKg) + 1e-9) / weight);
    return {
      colour: stock.colour,
  ...
  let remaining = pcs - allocations.reduce((sum, row) => sum + row.plannedPcs, 0);
  while (remaining > 0) {
    const candidate = allocations
      .filter((row) => row.plannedPcs < row.capacity)
      .sort((a, b) => b.capacity - b.plannedPcs - (a.capacity - a.plannedPcs))[0];
    if (!candidate) break;
  ...
  if (remaining > 0) {
    ...
    throw new RangeError(`Fabric shortage: only ${availablePcs} PCS can be planned`);
  }
